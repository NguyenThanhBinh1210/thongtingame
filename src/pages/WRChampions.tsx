...
const WRChampions = () => {
  ...
  const filteredItems = useMemo(() => {
    const filtered = champions?.filter((champion) => {
      const matchesSearch =
        ...
        champion.roles.map(tag => tag.toLowerCase()).includes(searchTerm.toLowerCase())
  ...
  useQuery({
    queryKey: ['wr-champions'],
    queryFn: async () => {
      const response = await championApi.getWRChampions({ lang })
      if (response.data.items) {
        setChampions(response.data.items)
      }
    }
  })
  const langMutation = useMutation({
    mutationFn: (lang: string) => {
      return championApi.getWRChampions({ lang })
    },
    onSuccess: (data) => {
      setChampions(data.data.items)
    },
    onError: () => {
    ...
    <div>
      <div className='flex justify-between items-center mb-4'>
        ...
        <div className='flex gap-4 items-center'>
          ...
          <Input
            className='w-[300px]'
            placeholder='Tìm kiếm theo tên tướng hoặc roles...'
      ...
      <Table className='mt-4' isHeaderSticky isStriped aria-label='table'>
        <TableHeader>
          ...
          <TableColumn className='min-w-[300px]'>Roles</TableColumn>
              ...
              <TableRow key={index + 1}>
                ...
                <TableCell>
                  <div className=' uppercase flex gap-x-2 items-center'>
                    ...
                    <Button
                      onClick={() => {
                        navigate(`/wr-champions/edit/${item._id}`, {
                          state: item
                        })
                      }}
                      size='sm'
